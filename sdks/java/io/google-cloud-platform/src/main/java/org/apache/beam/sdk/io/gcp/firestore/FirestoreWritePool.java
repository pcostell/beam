...
import org.apache.beam.sdk.io.gcp.firestore.FirestoreWritePool.DbWritePool;
 ...
 */
final class FirestoreWritePool<OutT> {
  private static final Logger LOG =
      LoggerFactory.getLogger(V1FnRpcAttemptContext.BatchWrite.getNamespace());
  private final JodaClock clock;
  private final FirestoreStatefulComponentFactory firestoreStatefulComponentFactory;
  private final RpcQos rpcQos;
  private final Counter writesSuccessful;
  private final Counter writesFailedRetryable;
  private final Counter writesFailedNonRetryable;
  @VisibleForTesting
  protected final Map<DatabaseRootName, DbWritePool> dbWritePools = new HashMap<>();
  private final PipelineOptions pipelineOptions;
  private final SuccessHandler<OutT> successHandler;
  private final FailureHandler<OutT> failureHandler;
  ...
  FirestoreWritePool(
      ...
      SuccessHandler<OutT> successHandler,
      FailureHandler<OutT> failureHandler) {
    this.clock = clock;
    this.firestoreStatefulComponentFactory = firestoreStatefulComponentFactory;
    this.rpcQos = firestoreStatefulComponentFactory.getRpcQos(rpcQosOptions);
  ...
  public void close(ContextAdapter<OutT> contextAdapter) throws InterruptedException {
    try {
      for (DbWritePool pool : dbWritePools.values()) {
        pool.flush(contextAdapter);
      }
    } finally {
      for (DbWritePool pool : dbWritePools.values()) {
        pool.close();
  ...
  public void write(Write write, BoundedWindow window, ContextAdapter<OutT> contextAdapter)
    ...
    if (tooLarge) {
      ...
      getDbWritePool(write).write(write, window, contextAdapter);
    }
  }

  @VisibleForTesting
  ArrayList<WriteElement> getAllOutstandingWrites() {
    ArrayList<WriteElement> outputQueue = new ArrayList<>();
    for (DbWritePool pool : dbWritePools.values()) {
      outputQueue.addAll(pool.writes);
    }
    return outputQueue;
  }

  private DatabaseRootName getDatabaseRoot(Write w) {
    DocumentPathName docPath = DocumentPathName.parse(getName(w));
    return DatabaseRootName.of(docPath.getProject(), docPath.getDatabase());
  }

  private DbWritePool getDbWritePool(Write w) {
    return dbWritePools.computeIfAbsent(getDatabaseRoot(w), dbRoot -> new DbWritePool(dbRoot));
  }
  ...
  final class DbWritePool {
    private final DatabaseRootName dbRootName;
    private final FirestoreStub firestoreStub;
    private final Queue<@NonNull WriteElement> writes;
    @VisibleForTesting
    protected int lastQueuePriority = 0;

    DbWritePool(DatabaseRootName dbRootName) {
      this.dbRootName = dbRootName;
      this.firestoreStub =
          firestoreStatefulComponentFactory.createFirestoreStub(pipelineOptions, dbRootName);
      this.writes = new PriorityQueue<>(WriteElement.COMPARATOR);
      this.lastQueuePriority = 0;
    }

    void write(Write write, BoundedWindow window, ContextAdapter<OutT> context)
        throws InterruptedException {
      writes.add(new WriteElement(lastQueuePriority++, write, window));
      maybeFlush(false, context);
    }

    void flush(ContextAdapter<OutT> context) throws InterruptedException {
      maybeFlush(true, context);
    }
    ...
    FlushBuffer<WriteElement> getFlushBuffer(RpcWriteAttempt attempt, Instant start) {
    ...
    BatchWriteRequest getBatchWriteRequest(FlushBuffer<WriteElement> flushBuffer) {
      BatchWriteRequest.Builder commitBuilder =
          BatchWriteRequest.newBuilder().setDatabase(dbRootName.toString());
     ...
     */
    void maybeFlush(boolean finishingBundle, ContextAdapter<OutT> context)
        throws InterruptedException {
      while (!writes.isEmpty()) {
        RpcWriteAttempt attempt = rpcQos.newWriteAttempt(V1FnRpcAttemptContext.BatchWrite);
  ...
  /**
   * Window values are part of the WriteElement which is used to full the flushBuffer and accessible
   * after the response is returned. Our FlushBuffer is ensured to be non-empty before passed to
   * this method, so the loop above will always iterate at least once and by virtue of this method
   * only being when at least one ok status OK is present successfulWindow will be non-null.
   *
  ...
  /**
   * Adapter interface which can handle write success summaries.
   *
   * @param <OutT> the output type.
   */
  interface SuccessHandler<OutT> {

    void handle(
        ContextAdapter<OutT> context,
        Instant timestamp,
        KV<WriteSuccessSummary, BoundedWindow> outputSummary,
        Runnable logMessage);
  }

  /**
   * Adapter interface which can handle write failures.
   *
   * @param <OutT> the output type.
   */
  interface FailureHandler<OutT> {

    void handle(
        ContextAdapter<OutT> context,
        Instant timestamp,
        List<KV<WriteFailure, BoundedWindow>> failures,
        Runnable logMessage);
  }
   ...
   */
  interface ContextAdapter<OutT> {
    void output(OutT t, Instant timestamp, BoundedWindow window);

    static <OutT> ContextAdapter<OutT> forProcessContext(DoFn<Write, OutT>.ProcessContext context) {
      return (t, timestamp, window) -> context.outputWithTimestamp(t, timestamp);
    }

    static <OutT> ContextAdapter<OutT> forFinishBundleContext(
        DoFn<Write, OutT>.FinishBundleContext context) {
      return (t, timestamp, window) -> context.output(t, timestamp, window);
    }
  }
}
